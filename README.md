...
默认输出到状态文件同目录的 `dashboard.html`。页面使用“今日任务、能力分析、技能地图、练习记录、错题本、申论答题本、战绩段位、成就墙”八栏目闭环。技能地图固定展示 70 项标准技能；练习记录可以排序；错题本可按模块筛选并选择具体题目，同时保留易错点；成就墙展示五类 234 项成就。实力勋章以 11 项双战线五档阶梯显示，已达档位使用不同颜色，并明确样本进度、下一档和剩余档位。

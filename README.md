...
默认输出到状态文件同目录的 `dashboard.html`。页面有技能总览、错题本、易错点、战绩、申论答题册和勋章墙六个栏目。技能总览固定展示 70 项标准技能；战绩页可以排序并展示题量、正确数、正确率和实际用时；错题本可按模块筛选并选择具体题目；勋章墙展示五类 234 项成就。实力勋章以 11 项双战线五档阶梯显示，已达档位使用不同颜色，并明确下一档和剩余档位。
